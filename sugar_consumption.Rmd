...
sugar_data[sugar_data$Location =="Türkiye",]$region <- "Turkey" 
...
The sequential blue colour scale is merely the default for a continuous variable, and will not necessarily work well for any given application.  
...
label_sf$region_label <- paste(label_sf$region, label_sf$Value)
label_sf$region_label <- str_replace_all(label_sf$region_label, " ", "\n")
...
```{r, fig.height = 7, fig.cap = "", out.extra = ""}
pal <- diverge_hcl(n = 20)

ggplot(polygon_sf) + theme_minimal() +
  geom_sf(aes(fill = Value)) + xlim(-25, 55) + ylim(-35, 75) +
  geom_sf_text(aes(label = paste(region, Value, sep = "\n"), geometry = centroid), 
               data = label_sf[!is.na(label_sf$Value), ], size = 2, na.rm = TRUE,
               lineheight = 1) +
  scale_fill_gradientn(colours = pal) + theme(legend.position = "right")
```
...
pal <- diverge_hcl(n = 20, l = c(50, 90))

ggplot(polygon_sf) + theme_minimal() +
  geom_sf(aes(fill = Value), linewidth = 0.5, colour = "white") +
  geom_text_repel(stat = "sf_coordinates", aes(label = region_label, geometry = centroid), 
                  
               data = label_sf[!is.na(label_sf$Value), ], size = 2, na.rm = TRUE,
               lineheight = 1.05, min.segment.length = 0, force = 1.5, max.time = 2,
               max.overlaps = 225, seed = 123) +
...
ggplot(xy_grid[!is.na(xy_grid$Value), ]) + theme_void() +
  ...
  geom_point(aes(X, Y, size = Value,fill = Value), shape = 21, colour = grey(0.5), stroke = 0.1) +
...
Wainer and Francolini (1980) discusses the feasibility of two-variable colour plots; it seems that these will not typically be very useful.  
...
Howard Wainer and Carl M. Francolini, 1980. An Empirical Inquiry Concerning Human Understanding of Two-Variable Color Maps. The American Statistician, Vol. 34, No. 2, pp. 81-93
